...
CREATE TEMP TABLE tmp_sources AS -- создаётся временная таблица
...
dwh_delta_insert_result AS ( -- делаем расчёт витрины по новым данным. Этой информации по клиентам в рамках расчётного периода раньше не было, это новые данные. Их можно просто вставить (insert) в витрину без обновления
    SELECT
            ...
            --T4.craftsman_id AS top_craftsman_id, -- самый популярный мастер (не использовать)
            T4.top_craftsman_id as top_craftsman_id, -- самый популярный мастер
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и применяем оконную функцию для определения самой популярной категории товаров, а так же определяем самого популярного мастера у заказчика
                        *,
                        ...
                        --RANK() OVER(PARTITION BY T2.customer_id ORDER BY count_order_customer_id DESC) AS rank_count_order (лишний rank)
                        FROM (
...
------------------------------------------------------------------------------------
                --) AS T4 WHERE T4.rank_count_order = 1 ORDER BY report_period -- условие помогает оставить в выборке первую по популярности категорию товаров (неверный where)
    ...
    SELECT
            ...
            FROM (
                SELECT     -- в этой выборке объединяем две внутренние выборки по расчёту столбцов витрины и (применяем оконную функцию для определения самой популярной категории товаров) определям  самого популярного мастера ручной работы у заказчика
                        *,
                        RANK() OVER(PARTITION BY T2.craftsman_id ORDER BY count_product DESC) AS rank_count_product--, -- самый популярный мастера ручной работы у заказчика
                        --RANK() OVER(PARTITION BY T2.customer_id ORDER BY count_order_customer_id DESC) AS rank_count_order -- самая популярная категория товаров (лишний rank)
                        FROM (
                            SELECT -- в этой выборке делаем расчёт по большинству столбцов, так как все они требуют одной и той же группировки, (кроме столбца с самой популярной категорией товаров у мастера и самого популярного мастера у заказчика. Для этих столбцов сделаем отдельные выборки с другой группировкой и выполним 2 join)
                                T1.customer_id AS customer_id,
                                T1.customer_name AS customer_name,
                                T1.customer_address AS customer_address,
                                T1.customer_birthday AS customer_birthday,
...
------------------------------------------------------------------------------------
                            	...
                            	inner join (
									...
									order by dd.customer_id, count_order_customer_id desc) as T5 on T2.customer_id = T5.customer_id_for_craftsman_id
------------------------------------------------------------------------------------
                --) AS T4 WHERE T4.rank_count_order = 1 ORDER BY report_period --неверный where
				) AS T4 WHERE T4.rank_count_product = 1 ORDER BY report_period
),
--проверка
--select *
